...
def rnn_model(features, mode, params):
    ...
    x = tf.reshape(features[TIMESERIES_COL], [-1, N_INPUTS, 1]) # last "1" for the number of feature space here we have only one time dependent featureraa
    print('rnn_model iterations : x :{}'.format(x))
    # 2. configure the RNN
...
def rnn2_model(features, mode, params):
    ...
    #printing type of state in the notebook
    print('rnn2_model iterations : x :{}'.format(x))
    ...
    print('rnn2_model iterations : state :{}'.format(state))
    ...
    print('rnn2_model iterations : h1 :{}'.format(h1))
    predictions = tf.layers.dense(h1, 1, activation=None)  # (?, 1)
    return predictions
